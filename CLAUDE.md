...
Backends: `nearai` (default), `openai`, `anthropic`, `github_copilot`, `ollama`, `openai_compatible`, `tinfoil`, `bedrock` (requires `--features bedrock`) — set via `LLM_BACKEND`. `github_copilot` supports GitHub device login during onboarding. See [src/llm/CLAUDE.md](src/llm/CLAUDE.md) for per-provider auth and configuration details.
